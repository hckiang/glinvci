!----------------------------------------------------------------------|
      recursive subroutine DGPADM( ideg,m,t,H,ldh,wsp,lwsp,ipiv,iexph,ns,iflag )
...
!                      0   - no problem
!                     -9   - H is null
!                     -100 - Invalid parameter: ldh
!                     -200 - Invalid parameter: lwsp
!                     other- DGESV failure. If positive then matrix is
!                            numerically singular
!
!----------------------------------------------------------------------|
!     Edited by Hao Chi Kiang (hao.chi.kiang@liu.se)
...
!---  check restrictions on input parameters ...
      mm = m*m
      iflag = 0
      if ( ldh.lt.m ) iflag = -100
      if ( lwsp.lt.4*mm+ideg+1 ) iflag = -200
...
!     and set scale = t/2^ns ...
      ...
      enddo
      hnorm = ABS( t*hnorm )
      if ( hnorm.eq.0.0d0 ) iflag=-90 !stop 'Error - null H in input of DGPADM.'
